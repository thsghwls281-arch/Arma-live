...
export async function POST(req:NextRequest){
 try{
  ...
  const key=isAll?"global-ranking:v1":`sector-ranking:v2:${sector!.id}`;
  ...
  const base={ok:true,asOf:new Date().toISOString(),scope:isAll?"all":"sector",sectorId:isAll?"__all__":sector!.id,sectorName:isAll?"전체 종목":sector!.name,candidateCount:symbols.length,calculatedCount:rows.length,failedCount:symbols.length-rows.length,aScore:market.aScore,mScore:market.mScore,regime:market.regime,provisional:true};
