...
export function SectorOverview(){
 ...
 return <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
  ...
  <div className="mt-3 grid gap-2 md:grid-cols-2">{(open?data.sectors:sectors).map((s:any)=><article key={s.sector} className="rounded-2xl bg-slate-50 p-3"><div className="flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><span className={`h-2.5 w-2.5 shrink-0 rounded-full ${toneClass(String(s.tone||""))}`}/><b className="truncate">{s.sector}</b><span className="text-[10px] font-black text-slate-400">{s.status_label}</span></div><b className="text-xl text-blue-600">{Number(s.sector_score).toFixed(1)}</b></div><div className="mt-2 grid grid-cols-4 gap-1 text-center"><Mini code="SLS" label="섹터" value={s.sls}/><Mini code="R" label="추세" value={s.r_score}/><Mini code="BREADTH" label="확산" value={`${Number(s.breadth).toFixed(0)}%`}/><Mini code="ROTATION5D" label="5일" value={`${Number(s.rotation_5d)>0?"+":""}${Number(s.rotation_5d).toFixed(1)}`}/></div><p className="mt-2 text-[11px] font-bold text-slate-400">외국인 {flowArrow(s.foreign_net_buy)} · 기관 {flowArrow(s.institution_net_buy)} · 위험경고 {Number(s.prs).toFixed(0)}</p></article>)}</div>
