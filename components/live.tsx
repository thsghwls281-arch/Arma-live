...
export default function Live(){
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
  <header><div className="text-xs font-black tracking-widest text-blue-600">ARMA LIVE · KB CURRENT PRICE</div><h1 className="mt-2 text-4xl font-black">ARMA LIVE</h1><p className="mt-2 text-sm text-slate-500">KB 현재가 기준 전체 LIVE TOP20과 섹터별 TOP5를 계산합니다.</p><MarketRiskBadge/></header>
  ...
  {mode==="single"?<>
   ...
   <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black text-blue-600">LIVE RANKING</p><h2 className="text-xl font-black">{isAll?"전체 종목 TOP20":selectedSector.name}</h2><p className="mt-1 text-xs font-bold text-slate-400">{isAll?`등록 후보 ${STOCKS.length}종목`:"대표 후보 20종목"}</p></div><button onClick={runRanking} disabled={rankingBusy} className="shrink-0 rounded-2xl bg-blue-600 px-5 py-3 font-black text-white disabled:opacity-50">{rankingBusy?(isAll?"TOP20 계산 중":"TOP5 계산 중"):"랭킹 계산"}</button></div>
    <button onClick={()=>changeSector("__all__")} className={`mt-4 w-full rounded-2xl border px-4 py-4 text-left font-black transition ${isAll?"border-blue-600 bg-blue-50 text-blue-700":"border-slate-200 text-slate-600 hover:border-blue-300"}`}><span className="block text-base">전체 LIVE TOP20</span><small className="font-bold opacity-60">모든 등록 후보를 ARMA LIVE 점수순으로 정렬</small></button>
    ...
    <p className="mt-4 text-xs font-bold text-slate-400">수동 실행 · 시장 A/M은 공식 ARMA 상태로 단일 표시 · 섹터 신호는 공식 종가 스냅샷 · 종목·결과 5분 캐시</p>
   </section>
