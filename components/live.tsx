...
export default function Live(){
 ...
 const [sectorId,setSectorId]=useState(DEFAULT_SECTOR_ID),[ranking,setRanking]=useState<any>(null),[rankingBusy,setRankingBusy]=useState(false),[rankingError,setRankingError]=useState("");
 const singleMatches=useMemo(()=>matchStocks(q),[q]);
 const selectedSector=useMemo(()=>SECTOR_GROUPS.find(sector=>sector.id===sectorId)!,[sectorId]);
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
  <header><div className="text-xs font-black tracking-widest text-blue-600">INDEPENDENT · NO DATABASE</div><h1 className="mt-2 text-4xl font-black">ARMA LIVE</h1><p className="mt-2 text-sm text-slate-500">KB 현재가 기준 단일 종목과 10개 섹터별 LIVE TOP5를 계산합니다.</p></header>
  ...
  {mode==="single"?<>
   ...
   <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black text-blue-600">SECTOR LIVE RANKING</p><h2 className="text-xl font-black">{selectedSector.name}</h2><p className="mt-1 text-xs font-bold text-slate-400">대표 후보 20종목</p></div><button onClick={runRanking} disabled={rankingBusy} className="shrink-0 rounded-2xl bg-blue-600 px-5 py-3 font-black text-white disabled:opacity-50">{rankingBusy?"TOP5 계산 중":"랭킹 계산"}</button></div>
    <div className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-3">{SECTOR_GROUPS.map(sector=><button key={sector.id} onClick={()=>changeSector(sector.id)} className={`rounded-2xl border px-3 py-3 text-left text-sm font-black transition ${sector.id===sectorId?"border-blue-600 bg-blue-50 text-blue-700":"border-slate-200 text-slate-500 hover:border-blue-300"}`}><span className="block">{sector.name}</span><small className="font-bold opacity-60">후보 {sector.symbols.length}종목</small></button>)}</div>
    <p className="mt-4 text-xs font-bold text-slate-400">선택 섹터만 수동 실행 · A/M 공통 재사용 · 종목·결과 5분 캐시 · 자동 호출 없음</p>
   </section>
