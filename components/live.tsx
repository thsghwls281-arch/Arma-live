...
export default function Live(){
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
  ...
  {mode==="live"?<>
   ...
   <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="flex items-center text-xs font-black text-blue-600">실시간 순위<HelpTip code="RANKING"/></p><h2 className="text-xl font-black">전체 TOP10 · 매수 TOP10</h2><p className="mt-1 text-xs font-bold text-slate-400">등록 후보 {STOCKS.length}종목을 두 가지 기준으로 정렬</p></div><button onClick={runRanking} disabled={rankingBusy} className="shrink-0 rounded-2xl bg-blue-600 px-5 py-3 font-black text-white disabled:opacity-50">{rankingBusy?"랭킹 계산 중":"두 랭킹 계산"}</button></div><p className="mt-4 text-xs font-bold text-slate-400">전체 ARMA 점수순과 행동신호 ‘매수’ 종목의 ARMA 점수순을 함께 갱신합니다.</p></section>
