...
export default function Live(){
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
  ...
  {mode==="live"?<>
   ...
   <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="flex items-center text-xs font-black text-blue-600">실시간 매수 후보<HelpTip code="RANKING"/></p><h2 className="text-xl font-black">매수 신호 ARMA TOP10</h2><p className="mt-1 text-xs font-bold text-slate-400">등록 후보 {STOCKS.length}종목에서 매수 신호만 선별</p></div><button onClick={runRanking} disabled={rankingBusy} className="shrink-0 rounded-2xl bg-blue-600 px-5 py-3 font-black text-white disabled:opacity-50">{rankingBusy?"매수 후보 계산 중":"매수 TOP10 계산"}</button></div><p className="mt-4 text-xs font-bold text-slate-400">KB 현재가를 반영해 행동신호가 ‘매수’인 종목만 ARMA 점수순으로 표시합니다.</p></section>
