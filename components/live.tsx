...
export default function Live(){
 ...
 return <main className="mx-auto min-h-screen max-w-5xl px-4 py-8">
  ...
  {mode==="live"?<>
   ...
   <section className="mt-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between gap-3"><div><p className="flex items-center text-xs font-black text-blue-600">실시간 순위<HelpTip code="RANKING"/></p><h2 className="text-xl font-black">전체 실시간 TOP10</h2><p className="mt-1 text-xs font-bold text-slate-400">등록 후보 {STOCKS.length}종목</p></div><button onClick={runRanking} disabled={rankingBusy} className="shrink-0 rounded-2xl bg-blue-600 px-5 py-3 font-black text-white disabled:opacity-50">{rankingBusy?"TOP10 계산 중":"TOP10 계산"}</button></div><p className="mt-4 text-xs font-bold text-slate-400">KB 현재가와 공식 A/M 상태를 결합한 5분 잠정 순위입니다.</p></section>
...
function RankingResult({data,limit,watchlist,onToggle}:{data:any;limit:number;watchlist:string[];onToggle:(symbol:string)=>void}){const global=data.scope==="all";const [rankView,setRankView]=useState<"score"|"buy">("score");const rows=(global&&rankView==="buy"?data.buyTop10||[]:data.top20||data.top5||[]).slice(0,limit);return <section className="mt-4"><BasisBadge data={data}/><div className="mt-3 flex items-end justify-between px-1"><div><p className="text-xs font-black text-blue-600">{data.sectorName} · 후보 {data.candidateCount}종목</p><h2 className="text-2xl font-black">{global?(rankView==="buy"?"매수 신호 TOP10":`전체 현재가 TOP${limit}`):"섹터 현재가 TOP5"}</h2></div><div className="text-right text-xs font-bold text-slate-400">{data.cached?"5분 캐시 재사용":"새로 계산"} · {new Date(data.asOf).toLocaleTimeString("ko-KR")}</div></div>{global&&<nav className="mt-3 grid grid-cols-2 rounded-2xl bg-slate-200 p-1" aria-label="랭킹 기준"><button onClick={()=>setRankView("score")} className={`rounded-xl py-2.5 text-sm font-black ${rankView==="score"?"bg-white text-blue-600 shadow-sm":"text-slate-500"}`}>ARMA 점수순</button><button onClick={()=>setRankView("buy")} className={`rounded-xl py-2.5 text-sm font-black ${rankView==="buy"?"bg-white text-blue-600 shadow-sm":"text-slate-500"}`}>매수 신호</button></nav>}<div className="mt-3 space-y-3">{rows.map((stock:any,index:number)=><article key={stock.symbol} className="overflow-visible rounded-3xl bg-white shadow-sm"><div className="grid grid-cols-[42px_1fr_auto] items-center gap-3 p-4"><b className="text-2xl text-blue-600">{index+1}</b><div><div className="flex items-center gap-2"><h3 className="font-black">{stock.name}</h3><button onClick={()=>onToggle(stock.symbol)} aria-label={watchlist.includes(stock.symbol)?"관심종목 해제":"관심종목 저장"} className="text-xl text-amber-500">{watchlist.includes(stock.symbol)?"★":"☆"}</button></div><p className="text-xs text-slate-400">{stock.symbol} · {stock.sector} · {won(stock.price)}</p></div><div className="text-right"><b className="text-3xl text-blue-600">{stock.armaScore}</b><span className="flex items-center justify-end text-xs font-bold text-slate-400">{stock.action}<HelpTip code="Action"/></span></div></div><div className="grid grid-cols-4 border-t border-slate-100 bg-slate-50 p-3 text-center"><Score k="R" v={stock.rScore}/><Score k="AR" v={stock.arScore}/><Score k="PRS" v={stock.prs}/><Score k="SLS" v={stock.sls}/></div><TradePlan data={stock} compact/></article>)}{rows.length===0&&<div className="rounded-3xl bg-white p-8 text-center text-sm font-bold text-slate-400">현재 매수 신호 종목이 없습니다.</div>}</div><p className="mt-3 px-1 text-xs font-bold text-slate-400">실시간 잠정값 · 시장 위험색과 섹터 신호는 공식 ARMA 스냅샷을 사용합니다.</p></section>}
